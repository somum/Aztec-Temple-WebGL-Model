		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./Aztec.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
                            console.log('This is working');
						} else {
							loadImage('./Temple.png', function (imgErr, img) {
								if (imgErr) {
...
var RunDemo = function (vertexShaderText, fragmentShaderText, SusanImage, SusanModel) {
	...
	//mat4.lookAt(viewMatrix, [100/*camera view upor theke/nich theke shamne,piche diagonal*/,200/*camera view upor theke/nich theke shamne,piche*/, 150/*camera view upor theke, nich theke*/], [300/*left-right*/, -200/*diagonal*/, -700/*up-down*/], [0/*camera upor theke left right*/, 0/*camera upor theke updown*/,700]);
	mat4.lookAt(viewMatrix, [195,390, 55], [0, -500, -70], [-20, -190, 990]);
	mat4.perspective(projMatrix, glMatrix.toRadian(45), canvas.clientWidth/ canvas.clientHeight, 10.91, 1000.0);
	...
	canvas.onmouseout = function (ev) { click1(gl); };
	var flag = 0;
	function click(gl)
	{
	   if(flag ==0)
