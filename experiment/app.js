		...
		if (vsErr) {
			...
			loadTextResource('./shader.fs.glsl', function (fsErr, fsText) {
				if (fsErr) {
					...
					loadJSONResource('./project.json', function (modelErr, modelObj) {
						if (modelErr) {
							alert('Fatal error getting Susan model (see console)');
							console.error(fsErr);
						} else {
							loadImage('./textures.png', function (imgErr, img) {
								if (imgErr) {
...
var RunDemo = function (vertexShaderText, fragmentShaderText, SusanImage, SusanModel) {
	...
	mat4.lookAt(viewMatrix, [0, 0, -70], [0, 0, 0], [0, 1, 0]);
	...
	var loop = function () {
		...
		mat4.rotate(xRotationMatrix, identityMatrix, 80, [1, 0, 0]);
